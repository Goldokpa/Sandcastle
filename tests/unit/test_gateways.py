...
from sandcastle.testing import MockGateway, MockGatewayQueueEmptyError
...
class TestMockGateway:
    ...
    @pytest.mark.asyncio
    async def test_raises_when_queue_empty(self):
        mock = MockGateway()
        with pytest.raises(MockGatewayQueueEmptyError):
